-- Fix duplicate financial entries when converting leads to clients
-- Problem: When a lead with payment is converted to client, both lead_payment and client_valor_pago
-- entries are created, causing duplication.
-- Solution: The financial module now ignores lead_payment entries (only shows client_valor_pago)
