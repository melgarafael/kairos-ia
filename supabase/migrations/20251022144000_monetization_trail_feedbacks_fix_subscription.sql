-- Ajuste RPC: evitar inserir em saas_subscriptions quando organization_id for NULL
...
begin
  ...
  if coalesce(p_completed,false) = true and coalesce(p_bonus_requested,false) = true then
    ...
    if v_pro_plan_id is not null then
      ...
      -- Inserir assinatura trialing apenas quando houver organization_id válido
      if v_org_id is not null then
