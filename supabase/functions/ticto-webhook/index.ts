// Supabase Edge Function: ticto-webhook
// Env: MASTER_SUPABASE_URL, MASTER_SUPABASE_SERVICE_ROLE_KEY, TICTO_WEBHOOK_SECRET, TICTO_SIGNATURE_HEADER (opcional)
...
function getCorsHeaders(req: Request): HeadersInit {
  ...
  return {
    ...
    'Vary': 'Origin',
    'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, idempotency-key, x-ticto-signature, x-hmac-signature',
...
serve(async (req) => {
  ...
  const TICTO_WEBHOOK_SECRET = Deno.env.get('TICTO_WEBHOOK_SECRET')
  const SIGN_HEADER = Deno.env.get('TICTO_SIGNATURE_HEADER') || 'X-Ticto-Signature'
  if (!MASTER_SUPABASE_URL || !MASTER_SUPABASE_SERVICE_ROLE_KEY || !TICTO_WEBHOOK_SECRET) {
  ...
  try {
    const expected = await hmacSha256(TICTO_WEBHOOK_SECRET, rawBody)
    const ok = safeEqual(provided, expected)
    if (!ok) return new Response('Invalid signature', { status: 401, headers: getCorsHeaders(req) })
  } catch (e) {
  ...
  const externalId: string = pick(payload, 'subscription.id') || pick(payload, 'order.id') || pick(payload, 'id') || ''
  ...
  try {
    const statusRaw = (pick(payload, 'status', '') || pick(payload, 'subscription.status', '')).toString().toLowerCase()
    const isPaid = /paid|approved|active|completed/.test(statusRaw)
    const isCanceled = /canceled|cancelled|refunded|chargeback|inactive/.test(statusRaw)

    const email: string | undefined = pick(payload, 'customer.email') || pick(payload, 'buyer.email')
    const productCode: string | undefined = pick(payload, 'product.code') || pick(payload, 'subscription.product.code')
    const planSlugMeta: string | undefined = pick(payload, 'metadata.plan_slug')
    ...
    if (!planId && productCode) {
      const { data: p2 } = await supabase.from('saas_plans').select('id').eq('ticto_product_code', productCode).maybeSingle()
      planId = p2?.id || null
    }
    ...
    const externalSubscriptionId: string | null = pick(payload, 'subscription.id') || pick(payload, 'order.subscription_id') || null
